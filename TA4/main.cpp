...
int main(){
	...
	std::cout << "Enter num of vertices:" << std::endl;
	if(!(std::cin >> V)){
		std::cerr << "ERROR: Not an integer number" << std::endl;
		exit(1);
	}
	if(V <= 0){
	...
	{
		int u,v,weight;
		std::cout << "Enter edges with the next syntax:\n <u v weight>:" << std::endl;
		...
		if(weight < 0){
			std::cerr << "ERROR: Negative weight will not be tolerated" << std::endl;
			exit(1);
		}
